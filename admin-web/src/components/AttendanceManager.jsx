...
const AttendanceManager = ({ students = [], tenantId, onAlert = () => {}, onConfirm = () => {}, grades: propGrades, filterGrade, filterBatch, batches = {} }) => {
    ...
    const activeStudents = students.filter(s => 
        s.status === 'ACTIVE' && 
        (!filterGrade || filterGrade === 'All' || s.grade === filterGrade) &&
        (!filterBatch || filterBatch === 'All' || s.batch === filterBatch)
    );
    ...
    const saveAttendance = async () => {
        ...
        try {
            ...
            await setDoc(doc(db, 'attendance', docId), {
                tenantId,
                date: selectedDate,
                grade: 'All', // Set to All for shared visibility
                batch: 'All', // Set to All for shared visibility
    ...
    return (
        <div className="animate-fade-in" style={{ maxWidth: '1100px', margin: '0 auto' }}>
            <div className="glass-panel" style={{ padding: '24px', marginBottom: '32px', display: 'flex', justifyContent: 'space-between', alignItems: 'center', flexWrap: 'wrap', gap: '20px' }}>
                ...
                <div style={{ display: 'flex', alignItems: 'center', gap: '12px', background: 'rgba(255,255,255,0.03)', padding: '8px 16px', borderRadius: '12px', border: '1px solid var(--border)' }}>
                    ...
                    <input
                        ...
                        style={{ background: 'transparent', border: 'none', color: 'var(--text-primary)', fontSize: '1rem', outline: 'none', fontFamily: 'inherit' }}
                    />
                </div>
            </div>
            ...
            ) : (!tenantId || !students || students.length === 0 || activeStudents.length === 0) ? (
                ...
                <div className="glass-panel" style={{ padding: '0', overflow: 'hidden' }}>
                    <div style={{ padding: '20px 24px', borderBottom: '1px solid var(--border)', display: 'flex', justifyContent: 'space-between', alignItems: 'center', background: 'rgba(255,255,255,0.02)' }}>
                        <div style={{ display: 'flex', gap: '12px' }}>
                            <button className="btn btn-ghost" onClick={() => markAll('PRESENT')} disabled={isLocked()} style={{ color: 'var(--success)', fontSize: '0.85rem' }}>Mark All Present</button>
                            <button className="btn btn-ghost" onClick={() => markAll('ABSENT')} disabled={isLocked()} style={{ color: 'var(--danger)', fontSize: '0.85rem' }}>Mark All Absent</button>
                        </div>
                        {isLocked() && <div className="badge badge-warning" style={{ fontSize: '0.75rem' }}>🔒 Locked (Read Only)</div>}
                    </div>
                                    ...
                                    return (
                                        <tr key={student.id} style={{ borderBottom: '1px solid var(--border)', transition: 'background 0.2s' }}>
                                            ...
                                            <td style={{ padding: '20px 24px', textAlign: 'center' }}>
                                                <span className={`badge ${status === 'PRESENT' ? 'badge-success' : status === 'ABSENT' ? 'badge-danger' : status === 'LATE' ? 'badge-warning' : ''}`} style={{ opacity: status === 'UNMARKED' ? 0.3 : 1 }}>{status}</span>
                                            </td>
                                            <td style={{ padding: '20px 24px' }}>
                                                <div style={{ display: 'flex', justifyContent: 'center', gap: '8px' }}>
                                                    <button disabled={isLocked()} onClick={() => handleStatusChange(student.id, 'PRESENT')} style={{ width: '36px', height: '36px', borderRadius: '10px', border: '1px solid var(--success-border)', background: status === 'PRESENT' ? 'var(--success)' : 'transparent', color: status === 'PRESENT' ? '#fff' : 'var(--success)', cursor: 'pointer', fontWeight: 800, transition: 'all 0.2s', opacity: isLocked() ? 0.3 : 1, fontSize: '0.8rem' }}>P</button>
                                                    <button disabled={isLocked()} onClick={() => handleStatusChange(student.id, 'ABSENT')} style={{ width: '36px', height: '36px', borderRadius: '10px', border: '1px solid var(--danger-border)', background: status === 'ABSENT' ? 'var(--danger)' : 'transparent', color: status === 'ABSENT' ? '#fff' : 'var(--danger)', cursor: 'pointer', fontWeight: 800, transition: 'all 0.2s', opacity: isLocked() ? 0.3 : 1, fontSize: '0.8rem' }}>A</button>
                                                    <button disabled={isLocked()} onClick={() => handleStatusChange(student.id, 'LATE')} style={{ width: '36px', height: '36px', borderRadius: '10px', border: '1px solid var(--warning-border)', background: status === 'LATE' ? 'var(--warning)' : 'transparent', color: status === 'LATE' ? '#fff' : 'var(--warning)', cursor: 'pointer', fontWeight: 800, transition: 'all 0.2s', opacity: isLocked() ? 0.3 : 1, fontSize: '0.8rem' }}>L</button>
                                                </div>
                                            </td>
                                        </tr>
                                    );
