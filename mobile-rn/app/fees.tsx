...
export default function FeesScreen() {
    ...
    const { studentId, tenantId } = useLocalSearchParams<{ studentId?: string; tenantId?: string }>();

    useEffect(() => {
        const uid = studentId || auth.currentUser?.uid;
        if (!uid) return;

        const studentIdsArray = studentId?.includes(',') ? studentId.split(',') : [uid];

        let q;
        if (studentId && tenantId) {
            // Parent exploring a child's fees -> needs tenantId for Firestore rule
            q = query(collection(db, 'fees'), where('tenantId', '==', tenantId), where('studentId', 'in', studentIdsArray));
        } else {
            // Student exploring their own fees -> rule passes purely by studentId
            q = query(collection(db, 'fees'), where('studentId', '==', uid));
        }
